...
本项目希望把这些能力统一管理起来，让团队可以像管理代码、API 和微服务一样管理 AI Skill，并围绕安全、可用性、版本、调试和 Agent 分发建立工程化标准。
...
Skill Manager 关注的不是“如何写出一个能跑的 Prompt”，而是 **如何让 Skill 成为安全、可用、可调试、可演进、可分发的 AI 能力单元**。

核心思想包括：

| 关注点 | 核心问题 | 平台目标 |
| ---- | ---- | ---- |
| 安全 | Skill 能调用哪些模型、工具、MCP 和数据？权限边界是否清晰？ | 建立工具权限、运行沙箱、审计日志和租户隔离能力 |
| 可用性 | Skill 是否稳定、是否可重复执行、失败后是否可定位？ | 管理输入输出 Schema、测试集、评测结果、运行状态和异常信息 |
| 版本 | Skill 的 Prompt、Workflow、模型配置和工具依赖如何演进？ | 提供版本记录、变更追踪、发布状态和回滚依据 |
| 可调试 | Agent 执行过程为什么得到这个结果？中间调用是否可复盘？ | 沉淀 Workflow Trace、Tool Trace、Memory Trace 和日志观测能力 |
| Agent 分发 | Skill 如何被不同 Agent、团队、租户或行业场景复用？ | 建设 Registry、Marketplace、标签分类、安装和授权机制 |

换句话说，Skill Manager 要解决的是：

> 如何把零散的 Prompt 和 Agent 能力，沉淀为可以安全运行、持续迭代、方便调试并可规模化分发的 Skill 资产。
